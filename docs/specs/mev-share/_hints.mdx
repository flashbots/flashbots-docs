...
| `hash` | Share the transaction (or bundle) hash. If other hints are specified, omitting this hint will **disable** the auction entirely, as searchers rely on the tx hash to include these transactions in their bundles. |
