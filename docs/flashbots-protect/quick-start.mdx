...
Flashbots Protect screens all transactions to determine if they generate MEV. If your transaction does not generate MEV, it will automatically be sent to the public mempool for faster inclusion.

The following functions do not require MEV protection: `transfer`, `transferFrom`, `approve`, `(weth) withdraw`, `(weth) deposit`, `safe transfer (NFT)`.
