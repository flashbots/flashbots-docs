...
To specify which builders will receive your transactions, the `targetBuilder` parameter can be specified multiple times. To send transactions to all supported builders, set `targetBuilder=all`.
...
> :information_source: The Stable configuration currently shares only the `hash` from users' transactions. This may change over time as we gather more data and fine-tune the configuration to maximize benefits for our users.

### Opt Out

To explicitly **disable** MEV-Share and use Protect with full privacy, set _only_ the `hash` hint in your Protect RPC URL:
...
| Fast Execution | `https://rpc.flashbots.net?mevshare=enabled&hint=logs&hint=function_selector&hint=contract_address&hint=hash` |
| Max Speed & MEV Kickbacks (Low Privacy) | `https://rpc.flashbots.net?mevshare=enabled&hint=calldata&hint=logs&hint=function_selector&hint=contract_address&hint=hash` |
