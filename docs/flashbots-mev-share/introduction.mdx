...
Users send their transactions to a specialized actor called a MEV-Share Node. The MEV-Share Node selectively shares information about the user's transaction according to their privacy preferences. Seeing this information, searchers submit optimistic bundles to the MEV-Share Node to attempt to extract MEV from user's transactions without seeing the full transaction data. MEV-Share Nodes simulate each of these searcher bundles and forward the successful ones on to builders along with a condition that the user must be paid back 90% of the MEV their transactions create.

## How do I use MEV-Share?
The most simple way to use MEV-Share is by sending transactions to [Flashbots Protect](/flashbots-protect/rpc/quick-start), which leverages the [Flashbots MEV-Share Node](/flashbots-protect/mev-share). Wallet and application developers should check out the [Flashbots Protect MEV-Share section](/flashbots-protection/mev-share) for information about integrating and configuring the Flashbots MEV-Share Node.
