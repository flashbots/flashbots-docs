...
- let $F_P$ = `block.priorityFee`
- let $F_B$ = `block.baseFee`

$$
T_{min} = 30000 \cdot (F_P + F_B)
$$

This represents the **total surplus ETH** that the coinbase must have received _in addition to gas fees_ for all transactions in the bundle.
...
$$
T_{optimal} = 30000 * (F_P + F_B) + (P_{net} \cdot \gamma)
$$
...
However, off-chain data will never be as accurate as on-chain data. This is because other transactions may affect the state of the blockchain (e.g. the price of a trading pair) if they are placed before your bundle in the block. Unless you're the block builder, you have no way of knowing where in the block your transaction will be placed, so you have no way of knowing whether the state used to calculate your strategy's parameters (e.g. your trade amounts) is still accurate.

For instance, if you were to run the same arbitrage calculation algorithm _offchain_, which we've implemented _onchain_ in the [BlindBackrunLogic](https://github.com/flashbots/simple-blind-arbitrage/blob/main/src/BlindBackrunLogic.sol#L249-L258) contract, you'd probably get different results. Again, this is because prices can change in the block before your transaction is executed. The advantage of onchain searching is that you always have immediate access to the latest system state, so your calculations will always be accurate.
