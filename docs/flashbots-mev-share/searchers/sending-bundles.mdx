...
Note that bundles with transactions including the `hash` parameter are considered "unmerged", and will throw an error.
...
### Nest bundles (be a Flashbots MEV-Share Node)
...
Bundles merged by the MEV-Share Node can be simulated to check MEV profits. To simulate a bundle, we use the [`mev_simBundle`](/flashbots-auction/searchers/advanced/rpc-endpoint#mev_simbundle) endpoint. 

:::caution only merged bundles can be simulated

Bundle simulations can only be executed on merged bundles, which contain only signed transactions, or nested bundles which also contain only signed transactions. Bundles with transactions including the `hash` parameter are considered "unmerged", and will throw an error.
