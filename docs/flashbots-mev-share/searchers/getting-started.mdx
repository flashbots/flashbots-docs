...
To start searching on MEV-Share, you will first need to connect to a matchmaker. As a reminder, the matchmaker is responsible for receiving transactions and bundles from users, and selectively sharing information ("hints") about them with searchers. When a searcher wants to include a transaction in their bundle, they use that transaction's hash. The matchmaker replaces it with the original transaction before sending the bundle to a block builder.

## Connect to Matchmaker

Flashbots runs a matchmaker on Ethereum mainnet. The matchmaker has endpoints to receive transactions and bundles, as well as an SSE event stream endpoint which dispatches pending events/transactions to searchers.
