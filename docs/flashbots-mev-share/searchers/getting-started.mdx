...
The simplest way to connect to the Flashbots MEV-Share Node is to use a [client library](/flashbots-auction/searchers/libraries/mev-share-clients). For this guide, we'll refer to [mev-share-client-ts](https://github.com/flashbots/mev-share-client-ts). Several other clients are being developed.
...
That being said, existing strategies can still be employed. Users of MEV-Share have the option to share whichever parts of the transaction they deem appropriate, so strategies which use calldata or similar data as an input are still viable.

To maximally leverage the orderflow on MEV-Share, new searching strategies will have to be implemented to profitably leverage this orderflow in your bundles. More specifically, you can perform more of your searching on-chain instead of off-chain, or you can submit many bundles that probabilistically backrun the orderflow.
