...
const eventHandler = matchmaker.onShareTransaction((tx: PendingShareTransaction) => {
    /*
...
Each of these properties are optional; if a field is not present, it means that the transaction sender chose not to share that information.

---

Now that you've started listening to transactions, you're ready to start searching! Read on to the next page to learn [how to send bundles](/flashbots-mev-share/searchers/sending-bundles).
