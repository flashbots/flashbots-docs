...
```typescript
import Matchmaker, { PendingTransaction, StreamEvent } from '@flashbots/matchmaker-ts'

const matchmaker = new Matchmaker(authSigner, {chainId: 1, name: "mainnet"})

const eventHandler = matchmaker.on(StreamEvent.Transaction, (tx: PendingTransaction) => {
    /*
