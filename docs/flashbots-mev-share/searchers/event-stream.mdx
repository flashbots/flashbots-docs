...
Event history results are returned in chunks whose size are defined by `limit`, the maximum limit being specified in the [`/history/info` endpoint](/docs/flashbots-mev-share/searchers/event-stream#get-apiv1historyinfo).
