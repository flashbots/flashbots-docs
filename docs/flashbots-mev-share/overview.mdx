...
## Introduction
> The MEV-Share Matchmaker was renamed to the MEV-Share Node to better reflect the role we envision this actor will play in SUAVE. This change will be reflected in our documentation after June 2023.
...
**Wallet users** connected to Flashbots Protect do not have to do anything to benefit from MEV-Share. Transactions received by the Protect RPC are automatically forwarded to MEV-Share with the [Stable](/flashbots-protect/rpc/mev-share#stable-configuration) configuration.
...
To send transactions directly to the Flashbots MEV-Share node, use a [client library](https://github.com/flashbots/MEV-Share node-ts) or send a request to the [`sendPrivateTransaction`](/flashbots-auction/searchers/advanced/rpc-endpoint#eth_sendprivatetransaction) endpoint with the `privacy` parameter.
