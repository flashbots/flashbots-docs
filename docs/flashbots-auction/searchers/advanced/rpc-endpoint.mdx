...
The `auction` parameter opts the transaction into mev-share, allowing the caller to specify which data they want to share with searchers on mev-share.

Enabling the auction without explicitly setting hints (i.e. `hint=[]`) will send the transaction with the default hints (everything enabled except `calldata`).

| Param | Type Info | Description |
|-|-|-|
| `enable` | boolean | Setting to `true` will enable data-sharing with mev-share. Setting to `false` will send the transaction with full privacy. |
