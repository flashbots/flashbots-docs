...
The `flashbots_getBundleStats` JSON-RPC method returns stats for a single bundle. You must provide a bundleHash and the blockNumber, and the signing address must be the same as the one who submitted the bundle.
...
The following fields from this endpoint will be deprecated in the upcoming v2 release:
* `submittedAt`
* `sentToMinersAt`
* `isSimulated`
* `isSentToMiners`

`submittedAt` will simply be renamed to `receivedAt`, to clearly indicate when the request was received by the RPC endpoint.
