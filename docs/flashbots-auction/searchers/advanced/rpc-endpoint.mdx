...
By default, transactions are sent to the Flashbots MEV-Share Node with the default [Stable](/flashbots-protect/rpc/mev-share#stable-configuration) configuration. The `privacy` parameter allows you to specify your own privacy parameters.
...
Validity is used to specify refund address and refund percentage to pay kickback from the backrun of this transaction.

By default, kickback is paid to the signer of the transaction and 90% of the backrun value is sent to the user by default.
...
| Param | Type Info | Description |
|-|-|-|
| `refund` | Array of objects | Each entry in the array specifies address that should receive kickback from backrun and percent of the backrun value. |
