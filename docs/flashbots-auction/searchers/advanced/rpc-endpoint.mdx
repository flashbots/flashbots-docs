...
For advanced users, you can interact with the relay's RPC endpoint at `protect.flashbots.net`. The Protect API provides JSON-RPC methods for interfacing with Flashbots which are documented below:
...
`eth_sendPrivateTransaction` is used to send a single transaction to Flashbots. Flashbots will attempt to built a block with the transaction for the next 25 blocks. See [Private Transactions](/flashbots-auction/searchers/advanced/private-transaction.mdx) for more info.
...
```curl
curl -X POST -H "Content-Type: application/json" -H "X-Flashbots-Signature: 0x1234:0xabcd" --data '{"jsonrpc":"2.0","method":"eth_sendBundle","params":[{see above}],"id":1}' https://protect.flashbots.net
```
