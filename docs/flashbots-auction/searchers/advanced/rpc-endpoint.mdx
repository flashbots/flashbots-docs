...
`mev_sendBundle` uses a new bundle format to send bundles to MEV-Share. See [mev-share spec](https://github.com/flashbots/mev-share/blob/main/specs/mev_sendBundle.md) for more information, or check out the [Sending Bundles](/flashbots-mev-share/searchers/sending-bundles) page for a short guide.
...
`mev_simBundle` uses a new bundle format to simulate matched bundles on MEV-Share. See [mev-share spec](https://github.com/flashbots/mev-share/blob/main/specs/mev_simBundle.md) for more information.
...
```typescript
{
  ...
  params: [{
    ...
    preferences?: {
      ...
      auction?: {     // MEV-Share options; optional
        hint?: Array< // data about tx to share w/ searchers on mev-share
            "contract_address" |
            "function_selector" |
            "calldata" |
            "logs" |
...
The `auction` parameter changes the privacy parameters of the transaction.

Enabling the auction without explicitly setting hints (i.e. `hint=[]`) will send the transaction with the default [Stable configuration](/flashbots-protect/rpc/mev-share#stable-configuration).
...
| `targetBuilders` | Array of strings | Builders to grant permission to include the transaction in a block. |
