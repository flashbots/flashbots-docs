...
title: v0.5 (current)
---
_Note: v0.5 is scheduled for release on Feb 8th, 2022. Please reference the [release plan](https://docs.flashbots.net/flashbots-auction/releases/alpha-v0.5) for information on upgrading and configuration._
 ...
 * block building should be initated immediately if there is no block being built at the time.
 * block building should be queued for execution immediately after the current block building task if a block is being built at the time. 
